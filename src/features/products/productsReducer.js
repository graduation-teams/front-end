...
const fetchBySlug = createReducer({ ...listInitialState }, builder => {
    builder
        .addCase(createAction(AC_PRODUCT.PRODUCTS_FETCH_BY_SLUG_START), (state, action) => {
            return { ...state, pending: true };
        })
        .addCase(createAction(AC_PRODUCT.PRODUCTS_FETCH_BY_SLUG_SUCCESS), (state, action) => {
            return { ...state, pending: false, success: true, data: action.payload.responsive };
        })
        .addCase(createAction(AC_PRODUCT.PRODUCTS_FETCH_BY_SLUG_FAILED), (state, action) => {
            return { ...state, failed: true, success: false, pending: false, data: [] };
        })
        .addDefaultCase((state, action) => {
            return { ...state };
